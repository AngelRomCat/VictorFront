...
export default {
    ...
    async getRecipes() {
      try {
        const response = await fetch('https://victorback-production.up.railway.app/api/recipes-recipes')
    ...
    getImageUrl(path) {
      return `https://victorback-production.up.railway.app/${path}`
    },
    async deleteRecipe(id) {
      try {

      const response =  await fetch(`https://victorback-production.up.railway.app/api/recipes-recipes/${id}`,
         {
          method: 'DELETE',
